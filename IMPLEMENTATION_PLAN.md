...
| P1-13 | Display feeds: APS schedule + ShopView polls (5 min) into display-only cache for later dashboard overlay. | Haiku | P1-05 | §4.2, C24 | app/sync/resources/display.py | Schedule JSON cached and served at `/api/v1/jb2-schedule`; failure of this feed never affects other sync. |
...
| P3-10 | Finish-operation → outbox: time-ticket header ensure + detail (time or setup/cycle per P0-04 findings, pieces, reason code), routing-step patch (fields per P0-05 findings); rework posts to earlier step; **all via outbox writer, zero direct calls**. | S (**F reviews payload mapping vs findings**) | P3-04, P1-09 | §4.4, §6.6 | app/outbox/payloads.py | Grep proves no `jb2/client` import outside sync/outbox; fake-JB2 receives exactly the DD §4.4 table's writes for a walked scenario; idempotency keys deterministic. |
...
|---|---|---|---|
| R1 | `OrderRoutingUpdate` PATCH fields not writable as hoped (status derived, actuals rejected) → C7/C8 write-back degraded. | M/H | P0-05 probes before any code; DD's stated fallback (rely on time tickets alone) baked into P3-10 payload mapping; Fable sign-off. |
